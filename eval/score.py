...
def accuracy_reward(predict: str, ground_truth: str) -> float:
    ...
    if answer == 'None':
        answer = re.search(r"<answer>(?!.*<answer>)(.*?)</answer>", predict, re.DOTALL | re.MULTILINE)
        answer = answer.group(1).strip() if answer else "None"
        if answer == 'None':
            idx = predict.rfind('Answer:')
            if idx != -1:
                answer = predict[idx + len('Answer:'):]
                for ans in answer:
                    if ans.isalpha() or ans.isdigit():
                        answer = ans.upper()
                        break
            else:
                answer = predict[-1]
    if grade_answer(answer, ground_truth):
        ...
        reward = 1.0
    elif predict.strip().lower() == ground_truth.strip().lower():
        reward = 1.0
    else:
        reward = 0.0
